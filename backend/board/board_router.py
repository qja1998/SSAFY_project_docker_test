...
@app.get(path="/read",description="기본 게시판 - 게시판 조회")
async def read_all_post(db:Session = Depends(get_db)):
    return board_crud.list_all_post(db)

@app.get(path="/read/{post_no}",description="기본 게시판 - 특정 게시판 상세 조회",response_model=board_schema.Post)
async def read_post(post_no: int, db:Session=Depends(get_db)):
    return board_crud.get_post(post_no,db)

@app.put(path="/update/{post_ne}",description="기본 게시판 - 특정 게시글 수정")
...
@app.patch(path="/delete/{post_ne}",description="기본 게시판 - 특정 게시글 삭제")
